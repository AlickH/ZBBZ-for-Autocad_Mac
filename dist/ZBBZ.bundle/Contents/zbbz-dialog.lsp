...
(defun zbbz-dialog-chinese-p (/ locale_code)
  (setq locale_code (zbbz-dialog-locale-code))
  (or (= (substr locale_code 1 2) "ZH")
      (= (substr locale_code 1 2) "CH")))

(defun zbbz-dialog-localize-line (line)
  (if (zbbz-dialog-chinese-p)
    (progn
      (setq line (vl-string-subst "ZBBZ 坐标标注设置" "ZBBZ Coordinate Annotation Settings" line))
      (setq line (vl-string-subst "坐标系统" "Coordinate System" line))
      (setq line (vl-string-subst "当前坐标系" "Current Coordinate System" line))
      (setq line (vl-string-subst "世界坐标系" "World Coordinate System" line))
      (setq line (vl-string-subst "自定义坐标系" "Custom Coordinate System" line))
      (setq line (vl-string-subst "基准北坐标" "Base N" line))
      (setq line (vl-string-subst "基准东坐标" "Base E" line))
      (setq line (vl-string-subst "旋转角" "Rotation" line))
      (setq line (vl-string-subst "已知基点与角度" "Known Base Point && Angle" line))
      (setq line (vl-string-subst "已知两点校准" "Known Two-Point Pick" line))
      (setq line (vl-string-subst "绘制坐标网格" "Draw Coordinate Grid" line))
      (setq line (vl-string-subst "预览" "Preview" line))
      (setq line (vl-string-subst "仅用于显示预览。" "Preview is descriptive only." line))
      (setq line (vl-string-subst "行为" "Behavior" line))
      (setq line (vl-string-subst "交换 X/Y" "Swap X/Y" line))
      (setq line (vl-string-subst "创建组" "Group On" line))
      (setq line (vl-string-subst "自动定向" "Auto Orient" line))
      (setq line (vl-string-subst "前缀" "Prefix" line))
      (setq line (vl-string-subst "无" "None" line))
      (setq line (vl-string-subst "样式" "Style" line))
      (setq line (vl-string-subst "标注图层" "Annotation Layer" line))
      (setq line (vl-string-subst "*当前*\n0" "*CURRENT*\n0" line))
      (setq line (vl-string-subst "箭头样式" "Arrow Style" line))
      (setq line (vl-string-subst "三角箭头\n无" "triangle\nnone" line))
      (setq line (vl-string-subst "箭头大小" "Arrow Size" line))
      (setq line (vl-string-subst "文字样式" "Text Style" line))
      (setq line (vl-string-subst "精度" "Precision" line))
      (setq line (vl-string-subst "方位角" "Bearing" line))
      (setq line (vl-string-subst "选择" "Pick" line))
      (setq line (vl-string-subst "标注比例" "Annotation Scale" line))
      (setq line (vl-string-subst "文字高度" "Text Height" line))
      (setq line (vl-string-subst "背景遮罩" "Background Mask" line))
      (setq line (vl-string-subst "导出 DAT 文件" "Generate DAT File" line))
      (setq line (vl-string-subst "确定" "OK" line))
      (setq line (vl-string-subst "取消" "Cancel" line))
      (setq line (vl-string-subst "帮助" "Help" line))
      line)
    line))
